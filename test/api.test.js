...
(async () => {
  ...
  const part = (name, filename, mime, data) => Buffer.concat([
  ...
  // CSV injection guard
  const csvMod = require(require('path').join(__dirname, '..', 'server', 'csv.js'));
  ...
  ok('admin payments list', adminPay.data.ok && adminPay.data.items.some((p) => p.order_code === code && p.status === 'paid'));
  ...
  ok('sms: all mock messages marked sent', box.data.items.every((m) => m.status === 'sent'));
  const orderSms = box.data.items.find((m) => m.kind === 'order' && m.body.includes(code));
  ...
  // status change → sms
  const oid = adminPay.data.items.find((p) => p.order_code === code).order_id;
  await api('PATCH', `/api/admin/orders/${oid}`, { status: 'shipping' }, { as: 'admin' });
  const box2 = await api('GET', '/api/admin/sms', null, { as: 'admin' });
  const ship = box2.data.items.find((m) => m.order_id === oid && /хүргэлтэд гарлаа/.test(m.body));
