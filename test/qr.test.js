'use strict';
// Round-trip check: decode qr.js output back to the original string.
// Verifies data placement, masking, format info, block interleaving.

const path = require('path');
const qr = require(path.join(__dirname, '..', 'server', 'qr.js'));
...
function decode(text) {
  ...
  const [ecLen, g1b, g1d, g2b, g2d] = qr.EC_M[version];
  const blockSizes = [];
...
for (const c of CASES) {
  try {
    ...
    console.log(`  FAIL  "${c.slice(0, 34)}" -> ${e.message}`);
    fail++;
  }
}

try { qr.encode("z".repeat(900)); console.log("  FAIL  over-capacity input did not throw"); fail++; } catch (e) { console.log("  PASS  over-capacity input rejected: " + e.message); pass++; }
